...
export default function EditarRegistro() {
  ...
  useEffect(() => {
    fetch(`/api/registros/${id}`)
      ...
      .then(json => {
        ...
        if (!r) { setError('Registro no encontrado.'); setLoading(false); return; }
        const NUMS = ['saldo_inicial','total_generado','efectivo','por_app','nafta','comida',
                      'otros_gastos','horas','pedidos','km','temperatura','energia'];
        const f = {};
        Object.keys(r).forEach(k => {
          if (NUMS.includes(k)) f[k] = r[k] != null ? String(r[k]) : '';
          else f[k] = r[k] ?? '';
        });
        f.notas = r.notas || '';
        setForm(f);
      ...
      .catch(() => { setError('Error cargando el registro.'); setLoading(false); });
  }, [id]);
  ...
  async function handleSubmit(e) {
    ...
    const res  = await fetch(`/api/registros/${id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) });
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <header style={{ borderBottom:`1px solid ${C.border}`, padding:'20px 32px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            ...
            <div style={{ fontSize:16, fontWeight:600, color:C.text, marginTop:4 }}>Editar registro</div>
          </div>
        ...
        <main style={{ padding:'32px', maxWidth:800 }}>
          {error && (
            <div style={{ border:`1px solid ${C.accent}`, color:C.accent, padding:'10px 14px', borderRadius:4, marginBottom:20, fontSize:13 }}>{error}</div>
          )}
          ...
          {loading ? (
            ...
            <form onSubmit={handleSubmit}>
              ...
              <Bloque title="Ingresos">
                <Grid4>
                  {[['Saldo inicial ($)','saldo_inicial'],['Total generado ($)','total_generado'],['Efectivo ($)','efectivo'],['Por app ($)','por_app']].map(([label, key]) => (
              ...
              <Bloque title="Estadísticas del turno">
                <Grid3>
                  <Campo label="Horas trabajadas"><input type="number" step="0.5" min="0" max="24" value={form.horas} onChange={set('horas')} style={inp} /></Campo>
                  <Campo label="Pedidos"><input type="number" step="1" min="0" value={form.pedidos} onChange={set('pedidos')} style={inp} /></Campo>
                  <Campo label="Kilómetros"><input type="number" step="0.1" min="0" value={form.km} onChange={set('km')} style={inp} /></Campo>
                </Grid3>
              </Bloque>
              ...
              <Bloque title="Condiciones">
                <Grid3>
                  ...
                  <Campo label="Temperatura (°C)"><input type="number" step="1" min="-10" max="50" value={form.temperatura} onChange={set('temperatura')} style={inp} /></Campo>
                  <Campo label="Energía (1–5)"><input type="number" step="1" min="1" max="5" value={form.energia} onChange={set('energia')} style={inp} /></Campo>
                </Grid3>
              </Bloque>
