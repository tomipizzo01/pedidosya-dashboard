...
export async function GET() {
  try {
    ...
    // W24 referencia B5/C5, W25→B6/C6, W26→B7/C7 ... W31→B12/C12
    // Fila W: 24 + i  →  B/C fila: 5 + i
    const data = [];
    for (let i = 0; i <= 7; i++) {
      const filaW   = 24 + i;
      const filaBnC = 5 + i;
    ...
    return NextResponse.json({
      ok: true,
      msg: '✅ Celdas W24:W31 corregidas — operadores & agregados',
      celdasCorregidas: data.map(d => ({
        celda: d.range.split('!')[1],
        formula: d.values[0][0],
      })),
    });
  } catch (e) {
